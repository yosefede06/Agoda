...
def cancellation_cost(booking_date, checkin_date, checkout_date, cancellation_date, original_selling_amount):
    ...
    days_before_checkin = (checkin_date - cancellation_date).days

    if days_before_checkin >= 30:
        cancellation_cost = 0.0  # No penalty if cancellation is made 30 or more days before check-in
    elif days_before_checkin >= 15:
        cancellation_cost = 0.1  # 10% penalty if cancellation is made between 15 and 30 days before check-in
    elif days_before_checkin >= 7:
        cancellation_cost = 0.3  # 30% penalty if cancellation is made between 7 and 14 days before check-in
    else:
        cancellation_cost = 0.5  # 50% penalty if cancellation is made less than 7 days before check-in

    cancellation_amount = original_selling_amount * cancellation_cost
...
def perform_eda(df):
    ...
    # Hypothesis testing
    # Perform hypothesis tests if required
...
def main_task4():
    X_cancel_train, y_cancel_train = clean_data_classifiers()
    perform_eda(X_cancel_train)
